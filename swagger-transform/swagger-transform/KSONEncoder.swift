...
/// `KSONEncoder` facilitates the encoding of `Encodable` values into JSON.
open class KSONEncoder {
    // MARK: Options
    ...
    open func encode<T : Encodable>(_ value: T) throws -> Data {
        let encoder = _KSONEncoder(options: self.options)
...
fileprivate class _KSONEncoder : Encoder {
    // MARK: Properties

    /// The encoder's storage.
    fileprivate var storage: _KSONEncodingStorage

    /// Options set on the top-level encoder.
    fileprivate let options: KSONEncoder._Options
    ...
    fileprivate init(options: KSONEncoder._Options, codingPath: [CodingKey] = []) {
        self.options = options
        self.storage = _KSONEncodingStorage()
    ...
    public func container<Key>(keyedBy: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        let container = _KSONKeyedEncodingContainer<Key>(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        return _KSONUnkeyedEncodingContainer(referencing: self, codingPath: self.codingPath, wrapping: topContainer)
    }
...
fileprivate struct _KSONEncodingStorage {
    // MARK: Properties
...
fileprivate struct _KSONKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    typealias Key = K

    // MARK: Properties

    /// A reference to the encoder we're writing to.
    private let encoder: _KSONEncoder
    ...
    fileprivate init(referencing encoder: _KSONEncoder, codingPath: [CodingKey], wrapping container: NSMutableDictionary) {
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type, forKey key: Key) -> KeyedEncodingContainer<NestedKey> {
        ...
        return self.with(pushedKey: key) {
            let container = _KSONKeyedEncodingContainer<NestedKey>(referencing: self.encoder, codingPath: self.codingPath, wrapping: dictionary)
            return KeyedEncodingContainer(container)
        }
    }
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        ...
        return self.with(pushedKey: key) {
            return _KSONUnkeyedEncodingContainer(referencing: self.encoder, codingPath: self.codingPath, wrapping: array)
        }
    }

    public mutating func superEncoder() -> Encoder {
        return _KSONReferencingEncoder(referencing: self.encoder, at: _KSONKey.super, wrapping: self.container)
    }

    public mutating func superEncoder(forKey key: Key) -> Encoder {
        return _KSONReferencingEncoder(referencing: self.encoder, at: key, wrapping: self.container)
    }
}

fileprivate struct _KSONUnkeyedEncodingContainer : UnkeyedEncodingContainer {
    // MARK: Properties
    ...
    /// Initializes `self` with the given references.
    fileprivate init(referencing encoder: _KSONEncoder, codingPath: [CodingKey], wrapping container: NSMutableArray) {
        self.encoder = encoder
        self.codingPath = codingPath
        self.container = container
    }
    ...
    public mutating func encode(_ value: Float)  throws {
        // Since the float may be invalid and throw, the coding path needs to contain this key.
        try self.encoder.with(pushedKey: _KSONKey(index: self.count)) {
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy keyType: NestedKey.Type) -> KeyedEncodingContainer<NestedKey> {
        ...
        return self.with(pushedKey: _KSONKey(index: self.count - 1)) {
            let container = _KSONKeyedEncodingContainer<NestedKey>(referencing: self.encoder, codingPath: self.codingPath, wrapping: dictionary)
            return KeyedEncodingContainer(container)
        }
    }
    ...
    public mutating func superEncoder() -> Encoder {
        return _KSONReferencingEncoder(referencing: self.encoder, at: self.container.count, wrapping: self.container)
    }
}

extension _KSONEncoder : SingleValueEncodingContainer {
...
extension _KSONEncoder {
    ...
    fileprivate func box<T : Encodable>(_ value: T) throws -> NSObject {
        if T.self == Date.self {
            ...
            let doubleValue = Double(decimalString)
//            let doubleValue = (value as! Decimal).doubleValue
            return try self.box(doubleValue)
        }

        // The value should request a container from the _KSONEncoder.
...
// MARK: - _KSONReferencingEncoder

/// _KSONReferencingEncoder is a special subclass of _KSONEncoder which has its own storage, but references the contents of a different encoder.
/// It's used in superEncoder(), which returns a new encoder for encoding a superclass -- the lifetime of the encoder should not escape the scope it's created in, but it doesn't necessarily know when it's done being used (to write to the original container).
fileprivate class _KSONReferencingEncoder : _KSONEncoder {
    ...
    fileprivate let encoder: _KSONEncoder
    ...
    fileprivate init(referencing encoder: _KSONEncoder, at index: Int, wrapping array: NSMutableArray) {
        self.encoder = encoder
        self.reference = .array(array, index)
        super.init(options: encoder.options, codingPath: encoder.codingPath)

        self.codingPath.append(_KSONKey(index: index))
    }

    /// Initializes `self` by referencing the given dictionary container in the given encoder.
    fileprivate init(referencing encoder: _KSONEncoder, at key: CodingKey, wrapping dictionary: NSMutableDictionary) {
        self.encoder = encoder
...
/// `KSONDecoder` facilitates the decoding of JSON into semantic `Decodable` types.
open class KSONDecoder {
    // MARK: Options
    ...
    open func decode<T : Decodable>(_ type: T.Type, from data: Data) throws -> T {
        let options = JSONSerialization.ReadingOptions(rawValue: 1 << 15)
        let topLevel = try JSONSerialization.jsonObject(with: data, options: options)
        let decoder = _KSONDecoder(referencing: topLevel, options: self.options)
...
fileprivate class _KSONDecoder : Decoder {
    // MARK: Properties

    /// The decoder's storage.
    fileprivate var storage: _KSONDecodingStorage

    /// Options set on the top-level decoder.
    fileprivate let options: KSONDecoder._Options
    ...
    fileprivate init(referencing container: Any, at codingPath: [CodingKey] = [], options: KSONDecoder._Options) {
        self.storage = _KSONDecodingStorage()
    ...
    public func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        ...
        let container = _KSONKeyedDecodingContainer<Key>(referencing: self, wrapping: topContainer)
    ...
    public func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        return _KSONUnkeyedDecodingContainer(referencing: self, wrapping: topContainer)
    }
...
fileprivate struct _KSONDecodingStorage {
    // MARK: Properties
...
fileprivate struct _KSONKeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol {
    typealias Key = K

    // MARK: Properties

    /// A reference to the decoder we're reading from.
    private let decoder: _KSONDecoder
    ...
    fileprivate init(referencing decoder: _KSONDecoder, wrapping container: [String : Any]) {
    ...
    public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: Key) throws -> KeyedDecodingContainer<NestedKey> {
        return try self.decoder.with(pushedKey: key) {
            ...
            let container = _KSONKeyedDecodingContainer<NestedKey>(referencing: self.decoder, wrapping: dictionary)
            return KeyedDecodingContainer(container)
        }
    }
    ...
    public func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
        return try self.decoder.with(pushedKey: key) {
            ...
            return _KSONUnkeyedDecodingContainer(referencing: self.decoder, wrapping: array)
        }
    }
    ...
    private func _superDecoder(forKey key: CodingKey) throws -> Decoder {
        return self.decoder.with(pushedKey: key) {
            ...
            return _KSONDecoder(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
        }
    }

    public func superDecoder() throws -> Decoder {
        return try _superDecoder(forKey: _KSONKey.super)
    }
...
fileprivate struct _KSONUnkeyedDecodingContainer : UnkeyedDecodingContainer {
    // MARK: Properties
    ...
    /// Initializes `self` by referencing the given decoder and container.
    fileprivate init(referencing decoder: _KSONDecoder, wrapping container: [Any]) {
        self.decoder = decoder
        self.container = container
        self.codingPath = decoder.codingPath
        self.currentIndex = 0
    ...
    public mutating func decodeNil() throws -> Bool {
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(Any?.self, DecodingError.Context(codingPath: self.decoder.codingPath + [_KSONKey(index: self.currentIndex)], debugDescription: "Unkeyed container is at end."))
        }
    ...
    public mutating func decode(_ type: Bool.Type) throws -> Bool {
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: self.decoder.codingPath + [_KSONKey(index: self.currentIndex)], debugDescription: "Unkeyed container is at end."))
        }

        return try self.decoder.with(pushedKey: _KSONKey(index: self.currentIndex)) {
            guard let decoded = try self.decoder.unbox(self.container[self.currentIndex], as: Bool.self) else {
                throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: self.decoder.codingPath + [_KSONKey(index: self.currentIndex)], debugDescription: "Expected \(type) but found null instead."))
            }
...
extension _KSONDecoder : SingleValueDecodingContainer {
...
extension _KSONDecoder {
    ...
    fileprivate func unbox(_ value: Any, as type: Bool.Type) throws -> Bool? {
        ...
        guard number.isBooleanObjCType else {
            throw DecodingError._typeMismatch(at: self.codingPath, expectation: type, reality: value)
        }
...
fileprivate struct _KSONKey : CodingKey {
    ...
    fileprivate static let `super` = _KSONKey(stringValue: "super")!
}
...
fileprivate extension EncodingError {
    ...
    fileprivate static func _invalidFloatingPointValue<T : FloatingPoint>(_ value: T, at codingPath: [CodingKey]) -> EncodingError {
        ...
        let debugDescription = "Unable to encode \(valueDescription) directly in JSON. Use KSONEncoder.NonConformingFloatEncodingStrategy.convertToString to specify how the value should be encoded."
